...
namespace Threax.Provision.CheapAzure.Controller.Create
{
    class CreateCertificate : IResourceProcessor<Certificate>
    {
        ...
        public Task Execute(Certificate resource)
        {
            ...
            keyVaultAccessManager.Unlock(config.KeyVaultName, config.UserId);
            ...
            using (var rsa = RSA.Create()) // generate asymmetric key pair
            {
                ...
                keyVaultManager.ImportCertificate(config.KeyVaultName, resource.Name, certBytes, password.ToSecureString());
            }
