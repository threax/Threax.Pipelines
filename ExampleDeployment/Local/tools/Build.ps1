...
docker run -it --rm -v "${pwd}:/out" threax-docker-tools-builder # This run line extracts the compiled tools from the image
Remove-Item -Recurse "$scriptPath/bin"
Move-Item "$scriptPath/portable" "$scriptPath/bin"
